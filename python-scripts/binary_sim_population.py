...
def arr_to_str(nparray: np.ndarray) -> str:
    """
    Takes a 1D numpy array of integers and joins them into a single string.

    Args:
        nparray: numpy.ndarray with ndim = 1.
...
def str_to_arr(string: str) -> np.ndarray:
    """
    Converts a string of integers into a 1D numpy array

    Args:
        string: String containing only 0 or 1.

    Returns:
        numpy.ndarray with ndim = 1 and dtype = np.integer.
    """
    return np.asarray(list(string), dtype=np.integer)


def seq_fitness(seq_string: np.ndarray, land: pd.DataFrame) -> float:
    """
    Get the fitness of the binary sequence from the landscape data frame.
    ...
    Args:
        ...
        seq_string: numpy.ndarray with ndim = 1.
            Binary sequence represented by a 1D numpy ndarray, where each element is 0 or 1.

    Returns:
        float
    """
    pep = arr_to_str(seq_string)
    try:
        fitness = land.at[pep, 'fit_norm']
    except KeyError:
...
def population_fitness(pop: np.ndarray, land: pd.DataFrame) -> list:
    ...
    Args:
        pop: numpy.ndarray with ndim = 2.
            2D numpy ndarray that contains all the binary sequences in a population.
    ...
    Returns:
        list
    """
    return [seq_fitness(pop[i], land) for i in range(pop.shape[0])]
...
def get_elites(pop: np.ndarray, population_metrics: list, land: pd.DataFrame) -> list:
    ...
    elite_fitness = [[e[0], arr_to_str(pop[e[0]]), seq_fitness(pop[e[0]], land)] for e in elite]
    return elite_fitness


class Population:
    def __init__(self, pop_size: int, seq_len: int, land: pd.DataFrame, rng_seed: object) -> None:
        """
        ...
        Args
            ...
            seq_len: int
                Length of the binary sequences. This should be obtained from the landscape file.
        ...
        pep_start = [self.rng.choice([0, 1]) for _ in range(self.seq_len)]
        self.population = np.broadcast_to(pep_start, (pop_size, seq_len)).copy()
        ...
        # Global peak in the landscape
        self.global_peak = land[land['global_peak'] == 1].reset_index()

        # Archive contains sequences that were novel. Used during novelty and combo search.
        self.archive = np.empty((0, seq_len), dtype=np.integer)
    ...
    def mutate(self, mut_rate: float = 1) -> None:
        ...
        # For each sequence in the population, mutate a random number of sites.
        for n in range(self.pop_size):
            ...
            if num_mut > 0:
                ...
                num_mut = min(num_mut, self.seq_len)
                # Select random indices to mutate.
                mut_indices = self.rng.choice(self.seq_len, num_mut, replace=False)
                # Mutate sites at the chosen indices.
                for i in mut_indices:
                    if self.population[n, i] == 0:
                        self.population[n, i] = 1
                    else:
                        self.population[n, i] = 0
        return
    ...
    def replace_pop(self) -> None:
        ...
        self.population = np.empty((0, self.seq_len), dtype=str)

        # Broadcast elites to 10% of pop size, then append.
        for seq in elite_nparray:
            self.population = np.append(self.population, np.broadcast_to(seq, (self.pop_size // 10, self.seq_len)),
                                        axis=0)
        self.population = self.population.astype('int32')
        return
