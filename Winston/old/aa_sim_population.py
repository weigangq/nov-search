...
def euclidean_distance(seq1: str, seq2: str) -> dict:
    """
    Calculate the distance between two amino acid sequences with the same length. Distance is based on polarity,
    hydropathy, isoelectric index, a combination of the 3 previous values, the normalized distances of the previous 4
    values, and the blosum matrix.
    ...
    dists = {'pol': 0, 'hydro': 0, 'iso': 0, 'all': 0, 'pol_norm': 0, 'hydro_norm': 0, 'iso_norm': 0, 'all_norm': 0,
             'blosum': 0}
    ...
    for key in dists:
        if key == 'blosum':
            ...
            dists[key] = sqrt(dists[key]) / len(seq1)
        else:
            dists[key] = dists[key] / len(seq1)
    return dists
...
def aa_fitness(aa_string: np.ndarray, land: pd.DataFrame) -> float:
    ...
    return fitness
    """
    return land.at[arr_to_str(aa_string), 'Fitness']
...
def population_fitness(pop: np.ndarray, land: pd.DataFrame) -> list:
    ...
    Returns:
        list
    """
    return [aa_fitness(pop[n], land) for n in range(pop.shape[0])]
        ...
        for n in range(self.pop_size):
            ...
            if num_mut > 0:
                ...
                for ind in mut_indices:
                    ...
                    possible_aa.remove(self.population[n, ind])
                    self.population[n, ind] = self.rng.choice(possible_aa)
        return
